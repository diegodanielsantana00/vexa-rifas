        ...
        children: [
          Expanded(
            child: FutureBuilder<String?>(
              future: DataLocal().readData(), // a previously-obtained Future<String> or null
              builder: (BuildContext context, AsyncSnapshot<String?> snapshot) {
                List<Widget> children;
                if (snapshot.hasData) {
                  return new FutureBuilder<Map>(
              future: RealTimeFireBase().getDataTrasations(
                  json.decode(snapshot.data!)[0]["Email"]), // a previously-obtained Future<String> or null
              builder: (BuildContext context, AsyncSnapshot<Map> snapshot2) {
                List<Widget> children;
                if (snapshot2.hasData) {
                  return new ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Column(
                        children: [
                          historyBuyColumns(context, snapshot2.data!.values.toList(), index),
                        ],
                      );
                    },
                  );
...
Widget historyBuyColumns(context, dynamic list, int index) {
  ...
  String minute = DateTime.parse(list[index]["dataTransacao"]).minute.toString();
  ...
  if (minute.length == 1) {
    minute = "0${DateTime.parse(list[index]["dataTransacao"]).minute.toString()}";
  }
  if (hour.length == 1) {
        ...
        child: Row(
          ...
          children: [
            Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
              BuildWidgets().buildTextFont(context, 12, FontWeight.w700, "$controllerSinal${list[index]["valorTotalPago"]} Créditos", colorCotroller400),
            ],),
            SizedBox(width: BuildWidgets().getSize(context).width*0.10,),
            Column(
              children: [
                SizedBox(
                  height: 6,
                ...
                BuildWidgets()
                    .buildTextFont(context, 12, FontWeight.w500, controllerText, Colors.black),
                SizedBox(
                  height: 6,
                ),
                BuildWidgets().buildTextFont(
                    context, 12, FontWeight.w500, "$hour:$minute h", Colors.black),
                SizedBox(
                  height: 2,
                ),
                BuildWidgets()
                    .buildTextFont(context, 15, FontWeight.w500, "$day/$month/$year", Colors.black)
              ],
            ),
          ],
        ),
